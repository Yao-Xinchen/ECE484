    ...
    with alive_bar(100, max_cols=140, manual=True) as safe_bar:
        ...
        while init_queue != []:
            ...
            if (partition_depth >= 5):
                print(
                    bcolors.OKBLUE + "If the % Safe bar hasn't gone up past 0% by now, we recommend exiting" + bcolors.ENDC)

            print(
                bcolors.BOLD + f"######## Current Partition Depth: {partition_depth}, car x: [{car_init[0][0]}, {car_init[1][0]}]  car v: [{car_init[0][3]}, {car_init[1][3]}]" + bcolors.ENDC)
            ...
            if not tree_safe(traces):
                ...
                if car_init[1][idx] - car_init[0][idx] < 0.01:
                    print(
                        bcolors.RED + f"Can't partition initial set dimension {idx} anymore. Scenario is likely unsafe😭" + bcolors.ENDC)
        ...
        if tmp.ndim < 2:
            ...
            if agent.id == 'car':
                ...
                dist_min, dist_max = get_extreme(
                    (state_dict['car'][0][0][1], state_dict['car'][0][0][2], state_dict['car'][0][1][1],
                     state_dict['car'][0][1][2]),
                    (state_dict['pedestrian'][0][0][1], state_dict['pedestrian'][0][0][2],
                     state_dict['pedestrian'][0][1][1], state_dict['pedestrian'][0][1][2]),
                )
                cont['ego.x'] = [
...
def eval_velocity(tree_list: List[AnalysisTree]):
    ...
    if (sum(velo_list) / len(velo_list) >= 7):
        print(
            bcolors.OKGREEN + f"Overall average velocity over {len(velo_list)} safe executions is {sum(velo_list) / len(velo_list)}. This is above the threshold of 7!😋" + bcolors.ENDC)
    else:
        print(
            bcolors.RED + f"Overall average velocity over {len(velo_list)} safe executions is {sum(velo_list) / len(velo_list)}. This is below the threshold of 7!😱" + bcolors.ENDC)
...
def combine_tree(tree_list: List[AnalysisTree]):
    ...
    final_trace = {
        agent_id: np.array(list(combined_trace[agent_id].values())).flatten().reshape((-1, trace[i].size)).tolist() for
        agent_id in combined_trace}
    root = AnalysisTreeNode(final_trace, None, tree_list[0].root.mode, None, None, node.agent, None, None, [], 0, 10,
