...
def eval_velocity(tree_list: List[AnalysisTree]):
    ...
    if (sum(velo_list) / len(velo_list) >= 6.5):
        print(
            bcolors.OKGREEN + f"Overall average velocity over {len(velo_list)} safe executions is {sum(velo_list) / len(velo_list)}. This is above the threshold of 6.5!😋" + bcolors.ENDC)
    else:
        print(
            bcolors.RED + f"Overall average velocity over {len(velo_list)} safe executions is {sum(velo_list) / len(velo_list)}. This is below the threshold of 6.5!😱" + bcolors.ENDC)
...
def combine_tree(tree_list: List[AnalysisTree]):
    ...
    final_trace = {agent_id: np.array(list(combined_trace[agent_id].values())).flatten().reshape(
        (-1, np.array(list(combined_trace[agent_id].values())).shape[-1])).tolist() for agent_id in combined_trace}
